"""Make a Unitree Go2 lower once, stand back up once, and disconnect."""
...
def require_aes_key(aes_key: str | None) -> str:
    ...
    try:
        ...
        raise SystemExit("The AES key must be exactly 32 hexadecimal characters.") from exc
    if len(key_bytes) != 16:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Lower and raise the Go2 once")
    ...
    try:
        ...
        confirmation = input(
            ...
            "Type SQUAT to lower once and stand back up once: "
        )
        if confirmation.strip() != "SQUAT":
            print("Cancelled. No action was requested.")
            return

        print("Lowering once...")
        robot.sport_command(1005)  # Unitree StandDown preset
        time.sleep(3)

        print("Standing up once...")
        robot.sport_command(1004)  # Unitree StandUp preset
        time.sleep(4)
